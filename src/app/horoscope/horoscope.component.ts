...
export class HoroscopeComponent implements OnInit {
  ...
  objectKeys = Object.keys;
	...
	constructor(private router: Router, private route: ActivatedRoute, public shareService: ShareService, public renderer: Renderer2, public horoService: HoroscopeService, private translate: TranslateService) {
	  Chart.register(...registerables);
		...
		//	.subscribe(res => {
		//		this.signs_v = res[0];
		//		this.o_signs_v = res[1];
		//		this.sign_imgs_v = res[2];
		//		this.rashis_v = res[3];
		//		this.o_rashis_v = res[4];
		//		this.rashi_lords_v = res[5];
		//		this.ruler_name_v = res[6];
		//		this.friend_pl_v = res[7];
		//		this.enemy_pl_v = res[8];
		//		this.aspects_v = res[9];
		//		this.house_traits_v = res[10];
		//		this.nakshatras_v = res[11];
		//	});
	  ...
	  //this.horoService.getJson('assets/data/rashi_lords.json')
		 // .subscribe(res => {
			//  this.rashi_lords_v = res;
		 // });
	  //this.horoService.getJson('assets/data/ruler_name.json')
		 // .subscribe(res => {
			//  this.ruler_name_v = res;
		 // });
	  //this.horoService.getJson('assets/data/friend_pl.json')
		 // .subscribe(res => {
			//  this.friend_pl_v = res;
		 // });
	  //this.horoService.getJson('assets/data/enemy_pl.json')
		 // .subscribe(res => {
			//  this.enemy_pl_v = res;
	  ...
	  //this.horoService.getJson('assets/data/house_traits.json')
		 // .subscribe(res => {
			//  this.house_traits_v = res;
		 // });
	  //this.horoService.getJson('assets/data/nakshatras.json')
		 // .subscribe(res => {
			//  this.nakshatras_v = res;
		 // });
	...
	ngOnInit() {
		...
		forkJoin(
			...
			.subscribe(dat => {
				...
				} else { this.horoService.getAstakvarga(this.binf.lat, this.binf.lng, this.binf.dob, this.binf.timezone, ofset, Number(ayn))
				   .subscribe(akv => {
				...
				} else { this.horoService.getShadbala(this.binf.lat, this.binf.lng, this.binf.dob, this.binf.timezone, ofset, Number(ayn))
				   .subscribe(shd => {
  ...
  akvChart(akv) {
			...
			this.hsChart = new Chart(this.cnvHS.nativeElement, {
			  type: "bar",
			  data: {
				labels: lbls,
				datasets: [
  ...
  loadHoro()
  {
	...
	console.log('signs_v', this.signs_v);
		...
		this.horoService.calcVim(this.binf.dob, this.bstar.split('|')[2], mdeg, Number(this.bstar.split('|')[1]), ras_num, ras_num2, this.shareService.getLANG() )
				.subscribe(res => {
					this.msg3 = '';
					this.vims = 'VIMSHOTTARI DASHA';
				    this.oDas = [];
	...
	publishRep()
	{
		...
		h_code += '<span>' + this.shareService.getPLSTR() + '</span>';
	   this.showVIM = true;
	   console.log('cur mdas', this.cur_m_das);
		...
		this.akashWani = '<h3>Analysis Of House & Zodiac Sign</h3>' + h_code;
	}
	days_of_a_year(year) {
