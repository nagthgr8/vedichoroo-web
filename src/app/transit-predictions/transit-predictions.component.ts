...
@Component({
  ...
  templateUrl: './transit-predictions.component.html',
  styleUrls: ['./transit-predictions.component.scss'],
})
export class TransitPredictionsComponent implements OnInit  {
   ...
   @ViewChild('tranChartFull', {static: true}) tranChartFull;
	...
	svgbf: any;
	svgtf: any;
  constructor(private router: Router, public shareService: ShareService, private horoService: HoroscopeService, public renderer: Renderer2, private eleRef: ElementRef) { 
  ...
  ngOnInit() {
	 ...
	 this.horoService.getBirthchartEx2(this.shareService.getCLAT(), this.shareService.getCLNG(), dob, this.shareService.getCTimezone(), ofset, ayanid)
	 .subscribe(res => {
	...
	console.log(plPos);
		...
		if(this.shareService.getCHTYP() == 'sind'){
			if(title == 'NATAL CHART') {
				this.svgHoro = this.grid(4, this.device_width/4, this.device_width, plPos, title, false, 'SZNCS');
			} else {
				this.svgHoro = this.grid(4, this.device_width/4, this.device_width, plPos, title, false, 'SZTCS');
			}
		}
		else if(this.shareService.getCHTYP() == 'nind') {
			if(title == 'NATAL CHART') {
				this.svgHoro = this.drawNIchart(plPos, title, this.device_width, false, 'SZNCS');
			} else {
				this.svgHoro = this.drawNIchart(plPos, title, this.device_width, false, 'SZTCS');
			}
		} else {
			if(title == 'NATAL CHART') {
				this.svgHoro = this.grid(4, this.device_width/4, this.device_width, plPos, title, false, 'SZNCS');
