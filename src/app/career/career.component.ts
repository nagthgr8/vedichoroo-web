...
export class CareerComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
	...
	forkJoin(
	  ...
	  this.shareService.getItem('lang').then(lang => { this.translate.use(lang as string); });
		...
		this.info = 'Getting birth star info...';
		   ...
		   this.dob = res['dob'];
			...
			this.shareService.getItem('lang').then(lang => {
			 this.horoService.calcVim(this.binf.dob, bstar.split('|')[2], mdeg, Number(bstar.split('|')[1]), ras_num, ras_num2
				...
				.subscribe(res => {
					...
					this.shareService.getItem('lang').then(lang => {
					 this.horoService.getCareerDas(this.mdas1.charAt(0).toUpperCase() + this.mdas1[1], this.binf.lat, this.binf.lng, this.binf.dob, this.binf.timezone, lang as string, ayanid)
						.subscribe(res2 => {
							this.info = '';
							this.showCD = true;
							let yogas = res2;
